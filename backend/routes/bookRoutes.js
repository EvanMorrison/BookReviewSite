...
.get(function(req, res) {
    Book.findOne({ISBN: req.ISBN}, function(err, book){
        if(err) res.status(500).send(err);
        res.send(book)
    })
}) 
...
.put(function(req, res) {
    Book.findOneAndUpdate({ISBN: req.ISBN}, req.body, {new:true}, function(err, editedBook) {
        if(err) res.status(500).send(err);
        res.send(editedBook)
    })
})
...
.delete(function(req, res) {
    Book.findOneAndRemove({ISBN: req.ISBN}, function(err, deletedBook){
        if(err) res.status(500).send(err)
